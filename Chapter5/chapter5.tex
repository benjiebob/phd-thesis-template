...
As discussed in \Cref{chap:relwork}, there is a long history of 3D reconstruction approaches which rely on morphable models. Examples of such priors have been demonstrated already, for faces \lazycite{Faces, rel-work}{Faces, rel-work}, SMPLify~\lazycite{SMPLify}{SMPLify} for human images, and in \Cref{chap:cgas} of this thesis, which uses the SMAL~\cite{zuffi2017menagerie} shape prior for monocular quadruped reconstruction. Often, these models are provided with 3D shape priors, which are used to help regularize fitting and thereby encourage plausible predictions. A typical strategy employed by these methods, is to design an energy (or loss) function that balances \emph{data terms} which encourages strong alignment between the 3D model and input image, and \emph{prior terms} which help to ensure 3D realism. Due to the limited 3D training data available for animals, existing priors are built using toy figurines rather than scans of real subjects so are consequently of much lower fidelity. While available animal priors are of sufficient quality to represent course differences between entire species (e.g. horse, camel, etc.), they poorly represent variation between members of the same species. For this reason, state of the art animal reconstruction approaches tend to produce similar looking output when tested on different examples of the same animal class. This raises an issue for practical applications of such systems; for example, animal researchers are unable to analyse subtle differences in weight and body proportions which could otherwise lead to health and wellbeing insights. 

The approach described in this chapter tackles this problem, importantly without requiring any additional 3D training data. Instead, a new technique is introduced which \emph{adapts an existing shape prior} by learning from large 2D image dataset. The proposed method takes inspiration from the SPIN method of Kolotouros et al., who demonstrate blending a deep convolutional neural network and an energy minimization framework. Their 3D reconstruction approach uses the HMR backbone~\lazycite{HMR}{HMR} to yield a set of SMPL~\lazycite{SMPL}{SMPL} human body shape and pose parameters $\Theta_{reg}$ from an input image. During SPIN's training phase, the initial fit $\Theta{reg}$ is processed by the SMPLify~\lazycite{SMPLify}{SMPLify} model fitting procedure, which refines the initial fit based on the ground truth joints to yield a new estimate $\Theta_{opt}$. The difference between these two predictions is then expressed as a loss $||\Theta_{reg} - \Theta_{opt}||$ which is backpropagated through the regression network to help it generate improved model parameter predictions.
...
This work demonstrates a novel technique for adapting a low-fidelity 3D shape prior to better represent the challenging dog category. The approach comprises an an expectation maximization process, applied during the training loop, and makes incremental updates to the prior based on weights learned from a large 2D image dataset. With no additional 3D training data required, this technique is shown experimentally to produce higher fidelity shape reconstructions. This has the effect of better recovering the ``personal'' details of each input dog, rather than crudely representing the breed. Further details on the expectation maximization process and overall reconstruction network are deferred to the later method section.

\subsection{Animal datasets}

In order to quantitatively compare to existing methods, and to stress test the fidelity of predicted 3D shapes, experimentation is based on \emph{StanfordExtra}, a new large-scale dataset containing 120 different dog breeds.
...
This thesis has already explored the use of 3D shape priors to regularize monocular reconstruction methods. 
