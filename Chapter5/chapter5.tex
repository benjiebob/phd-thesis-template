...
This chapter introduces a system for automatically recovering the 3D pose and shape of dogs from monocular internet imagery. As discussed in \Cref{chap:relwork}, there is a long history of 3D reconstruction approaches which rely on morphable models. Often, these models are provided with 3D shape priors, which are used to help regularize fitting and thereby encourage plausible predictions. Due to the limited 3D training data available for animals, existing priors are of relatively low fidelity, having been built from a few toy figurines rather than scans of real subjects. While available animal priors are of sufficient quality to represent obviously differences between entire species (e.g. horse, camel, etc.), they poorly represent the variation between members of the same species. For this reason, existing reconstruction approaches typically produce similar looking output even when tested on different examples of the same animal class. This proves to be an issue for the practical application of such systems, as it prevents animal researchers from analysing subtle differences in their animal populations with may have potential health implications. The approach described in this chapter targets this problem without requiring any additional 3D training data. Instead, a new technique is introduced which \emph{refines the shape prior} during the model's training procedure, and learns instead from a large 2D image dataset. In order stress test the fidelity of predicted shape, and quantitatively compare to existing methods, the system is trained and evaluated on a new large-scale dataset, containing 120 different dog breeds.

A further contribution of the work in this chapter is the design of the first 3D animal reconstruction system capable of real-time operation. The ability to reconstruct animals present in a live video stream is of great importance to future practical applications. For example, health or behaviour monitoring systems implemented as downstream tasks, could respond immediately to causes for concern among animal populations rather than having to act retrospectively. Presently, existing animal reconstruction pipelines incorporate per-image (or per-sequence) energy minimization procedures which prevent real-time use. The later experimentation section will show that the method presented in this chapter is of suitable quality that the refoutperforming such procedures, A benefit of the aforementioned approach for shape prior refinement is that the predicted animal models are already of suitable quality, making a later refinement stage optional.
