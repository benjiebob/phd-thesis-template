...
This chapter introduces a system for automatically recovering the 3D pose and shape of dogs from monocular internet imagery. As discussed in \Cref{chap:relwork}, there is a long history of 3D reconstruction approaches which rely on morphable models. Often, these models are provided with 3D shape priors, which are used to help regularize fitting and thereby encourage plausible predictions. Due to the limited 3D training data available for animals, existing priors are of relatively low fidelity, having been built from a few toy figurines rather than scans of real subjects. While available animal priors are of sufficient quality to represent obviously differences between entire species (e.g. horse, camel, etc.), they poorly represent the variation between members of the same species. For this reason, existing reconstruction approaches typically produce similar looking output even when tested on different examples of the same animal class. This proves to be an issue for the practical application of such systems, as it prevents animal researchers from analysing subtle differences in their animal populations with may have potential health implications. The approach described in this chapter targets this problem without requiring any additional 3D training data. Instead, a new technique is introduced which \emph{refines the shape prior} during the model's training procedure, and learns instead from a large 2D image dataset. In order to quantitatively compare to existing methods, and to stress test the fidelity of predicted 3D shapes, experimentation is based on \emph{StanfordExtra}, a new large-scale dataset containing 120 different dog breeds.

A further contribution of the system presented in this chapter is that it is capable of operating in real time. Being able to reconstruct animals in a live video stream is of great importance to potential downstream tasks, such as behaviour or health monitoring. Real-time processing potentially enables experts to immediately respond to causes for concern among their animal populations, rather than having to act on insights found retrospectively. At present, state-of-the-art animal reconstruction techniques incoporate per-image (or per-sequence as in \Cref{chap:cgas}) model fitting stages implemented as energy minimization procedures which prohibit their real-time use. Inspired by recent approaches in humans~\cite{kanazawa18end-to-end}, this chapter's approach incoporates a deep convolutional neural network that directly regresses the target parameters for the 3D morphable model. The shape prior refinement process, applied only at training time, has no impact on test time performance and, as shown in the experimentation section, enables the model to predict high quality 3D dog models that make a subsequent refinement step optional. 

\subsection{Designing a 3D shape prior}
even in monocular, single-image scenarios. Examples of such priors have been demonstrated already, for faces \lazycite{Faces, rel-work}{Faces, rel-work}, SMPLify~\lazycite{SMPLify}{SMPLify} for human images, and in \Cref{chap:cgas} of this thesis, which uses the SMAL~\cite{zuffi2017menagerie} shape prior for monocular quadruped reconstruction. A typical strategy employed by these methods, is to design an energy (or loss) function that balances \emph{data terms} which encourages strong alignment between the 3D model and input image, and \emph{prior terms} which help to ensure 3D realism. reconstruction strategy, employed by all these methods, is to balance a data term which encourages strong alignment between the 3D model and image data with a 3D shape prior to ensure realism. However, is to dHowever, the success of these priors relies heavily on the data used to build them. For animals, limited real-world 3D training data has resulted in the available models and priors being of relatively low fidelity, having been built instead from a few toy artist scans. While these priors are sufficient to approximate an animal species (e.g. an elephant, tiger, dog), they lack the representative power necessary for complex intra-species variation. Section XX demonstrates this problem by fitting the SMAL model to various dog images of different breeds. 

This is verified in chapter This can be seen by fitting the , fitting an existing quadruped model to various dog breeds the SMAL attempts to fit the SMAL model to different power to represent intricate intra-species variation. Of course, a successful er-specianimal species, they are certainly Monocular reconstruction pipelines To stress this, 

artist scans has As discussed previously, limited 3D training datadand which help regularize fitting to ensure plausible output models even when a only a single, monocular image is provided.  to ensure plausible particularly when only a single, monocular image is provided. Monocular 3D reconstruction pipelines typically rely on 3D shape priors in order help regularize fitting. fitting, approaches typically rely on 3D shape priors which help regularize However, there are relatively few techniques which 
The technique Differently to the work in \Cref{chap:cgas}, this chapter focuses on a technique for learning a rich 3D shape prior from a 2D dataset. 

2 focus of this method is to learn a rich 3D shape prior from a large 2D dataset capable of capturing the differences between individual dog breeds. 

llearning a rich 3D shape prior by to learn a rich 3D shape prior, enabling  focus of this approach is to learn a rich shape prior, capable of representing the differences between multiple dog breeds.




focussing on the dog category. on a single, challenging species: focussing on different examples of the same In this chapter, this phenomenon is stress tested by focussing with a new dataset This results in This results in reconstruction methods generating reconstruction methods being in reconstruction methods being being relatively This resulOf course, this could be resolved by collecting a much larger dataset of 3D models. overcome this challenge is to build priors on a larger dataset of 3D models. However, at the limit of this approach, it is clearly impractical to require a representative scan for the 3 billion species of animals on Earth. 


tThis is explored in depth in this chapter, in which SMAL~\cite{zuffi2017menagerie} struggles to represent the differences between different breeds of the same species. is fit to different breeds of dog, resulting in either poor resconstructions or a crude output shape a later section are poor at representing intra-species variation. For example, the SMAL~\cite{zuffi2017menagerie} could crudely used in \Cref{chap:cgas} was acceptable to crudely approximate the differences between bears, camels and dogs, but struggles to represent lack the representative power necessary to allow reconstruWhen applied to reconstruction pipelines, 


applying 3D morphable models to Unlike previous work in 3D morphable modelling, focus of the method introduces here  Differently to the work present in \Cref{chap:cgas}, the focus of this method is to learn a rich pthe wofocuses on learning a rich prior over animal shapes 
