...
As discussed in \Cref{chap:relwork}, there is a long history of 3D reconstruction approaches that rely on morphable models to represent articulated subjects. The most popular method is to parameterize the deformation space into pose (which governs limb movements) and a globally-defined PCA shape space. While efficient and differentiable, as noted by \lazycite{blackpaper}{}, this method of paramerizing a subject's shape characteristics is susceptible to learning unnatural correlations between body parts which coincidentally exist in the training corpus. Incidentally (although not futher explored in this chapter), the globally defined PCA space causes challenges for representing shape uncertainty, since each parameter governs multiple parts of the mesh. A variety of methods have been proposed to overcome this. One strategy, originally proposed in \lazycite{early paper} is to re-parameterize shape in terms of multiple localized PCA shape spaces (e.g. per body part) which can later be combined. However, the formulation can become expensive and it is not clear in the general case how to partition an 3D model. 

This chapter demonstrates an alternative approach, which adds local components to the existing globally-defined PCA shape space. The trick is to define per-body part scaling parameters, which have the effect of allowing that body part to vary in controlled manner, independently to the rest of the mesh. Experimentally, it can be shown this greatly helps when fitting the model to images poorly represented by the training data.
% A BIT MORE HERE. MAYBE ADD AN EXAMPLE?

\subsection{Automatic and real-time 3D dog reconstruction}

Early 3D reconstruction approaches in humans~\lazycite{examples}{examples} follow an energy minimization framework, in which a 3D template is optimized to match a single image (or sequence). However, more recent works tackle the reconstruction problem as a direct regression from the input image to 3D model parameters. For humans, deep learning approaches typically 3D morphable models (e.g. SMPL), 3D scans for learning priors, \emph{paired datasets} containing images and corresponding 3D annotations, and \emph{unpaired datasets} containing images and 2D annotations. Methods of this kind now outperform optimization-based approaches (e.g. SMPLify~\lazycite{SMPLify}{SMPLify}). A notable property of these ``end-to-end'' approaches, (i.e. deep learning only with no test-time optimization phase) is that they enable real-time inference. Occasionally, these methods are also evaluated in an `unpaired' setting, in which the paired datasets are ommited (or only 2D annotations are used). In such an unpaired setting, techniques must overcome fundamental ambiguities in the 2D-to-3D mapping, which can otherwise result in predicted bodies with impossible joint angles or are extremely skinny. This is typically achieved using learnt 3D priors, which ensure human body predictions lie on the manifold of plausible bodies. Further discussion on how to design a suitable prior for the dog category is covered in the next section.

Existing 3D animal reconstruction approaches are either entirely optimization based or incoporate optimization procedures as a part of the pipeline. Notable exceptions are the works of CMU, UCMU which focus on birds and are therefore unarticulated, and the work of \lazycite{BLAH} which does not learn animal shape characteristics. At present, there is no real-time 3D reconstruction system that recovers shape and pose for single-view, articulated animals. This restricts the future practical use of such reconstruction systems, as future downstream tasks such as animal behaviour and health analysis systems would be restricted to processing pre-recorded video, rather than being able to immediately respond to causes for concern. Inspired by the recent works in human mesh reconstruction, this work incorporates an end-to-end deep network which directly regresses 3D model parameters. Of concern, therefore, is how to design an accurate 3D shape prior given the lack of available training data. 
