...
Early 3D reconstruction approaches in humans~\lazycite{examples}{examples} follow an energy minimization framework, in which a 3D morphable model is iteratively optimized to match a single image or sequence. However, more recent works tackle the reconstruction problem as a direct regression from the input image to 3D model parameters. Most often, these approaches are implemented as deep neural networks and typically require 3D morphable models (e.g. SMPL), 3D scans for learning priors, \emph{paired datasets} containing images and corresponding 3D annotations, and \emph{unpaired datasets} containing images and 2D annotations. Methods of this kind now outperform optimization based approaches (e.g. SMPLify~\lazycite{SMPLify}{SMPLify}). A notable property of these ``end-to-end'' approaches, (i.e. deep learning only with no test-time optimization phase) is that they enable real-time inference. Occasionally, these methods are also evaluated in an `unpaired' setting, in which the paired datasets are ommited (or only 2D annotations are used). In such an unpaired setting, techniques must overcome fundamental ambiguities in the 2D-to-3D mapping, which can otherwise result in predicted bodies with impossible joint angles or are extremely skinny. This is typically achieved using learnt 3D priors, which ensure human body predictions lie on the manifold of plausible bodies. This unpaired scenario is of most relevance to this chapter's task of reconstructing 3D dogs, since paired 3D data is limited. Further discussion on how to design a suitable prior for the dog category is covered in the next section.

Existing 3D animal reconstruction approaches are either entirely optimization based or incoporate optimization procedures as a part of the pipeline. Notable exceptions are the works of CMU, UCMU which focus on birds and are therefore unarticulated, and the work of \lazycite{BLAH} which learns pose but not animal shape characteristics. At present, there is no real-time 3D reconstruction system that recovers shape and pose for single-view, articulated animals. Inspired by the recent works in human mesh reconstruction, this work incorporates an end-to-end deep network which directly regresses 3D model parameters. Of concern, therefore, is how to design an accurate 3D shape prior given the lack of available training data. 
