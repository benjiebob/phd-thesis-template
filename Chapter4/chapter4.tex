...
This chapter introduces a system for recovering the 3D shape and motion of a wide variety of quadrupedial animals from video. Inspired by the techniques from the recent human body and hand tracking literature (most notably SMPLify~\cite{bogo16keep}), the system comprises a machine learning front-end which predicts candidate 2D joint positions, a discrete optimization which finds kinematically plausible joint correspondences, and an energy minimization stage which fits a detailed 3D model to the image. However, the transfer from reconstructing 3D human subjects to animal subjects reveals a number of key challenges which transferring from reconstructing 3D human subjects to animal subjects is non-trivial and gives rise to a number of challenges. 
...
In the absence of real-world motion capture data then, the system incoporates as data generation method which allows training from synthetically generated images. Here, humans and animals share an advantage in the availability of parameterized 3D models of shape and pose.  The recent publication of the Skinned Multi-Animal Linear (SMAL) model~\cite{zuffi2017menagerie} can generate a wide range of quadruped species, although is far less detailed than the human counterpart and comes without surface texture maps.The use of synthetic images to train 3D reconstruction systems has been previously explored in the literature. The general approach is to capture training images by randomly sampling a camera viewpoint and rendering a representative 3D model. A machine learning model is then trained on these synthetic images and later tested on the real-world examples. This procedure offers an important practical advantage; the training images can be automatically generated with correponding annotations. Depending on the task, this can overcome a lengthy and complex process of sourcing manual annotations from users. However, designing a generation pipeline $G: (V, T) \mapsto \RRR{H}{W}{3}$ which can be applied to produce a synthetic dataset $D_{synth}$ with sufficient realism to be representative of the test image dataset $D_{real}$ is challenging. The underlying differences between datasets $D_{synth}$ and $D_{real}$ is often described as the \emph{domain gap} which must be bridged in order to achieve a successful predictor. 

Fortunately, there are a number of available methodologies for approaching this. One popular strategy is to ensure the generation pipeline $G$ is of extremely high quality. SURREAL\lazycite{SURREAL}{SURREAL} is a method which learns from synthetic human images rendered from the popular SMPL~\cite{loper15smpl} parametric model. In this work, the authors go to significant effort to source realistic UV texture maps for SMPL and apply a high quality rendering engine complete with a lighting and reflectance model to generate their training images. A similar approach is followed by SMALST, who follow a multi-view optimization pipeline to construct a synthetic zebra dataset complete with realistic textures. Related also is the work of \lazycite{GTA-Cars}{GTA-Cars} who use a images rendered from a popular video game engine to teach an automous agent to drive with reinforcement learning. 

An alternative option is to process the test images to find a representation which is easier to synthesise. In the case of humans, Shotton et al.~\cite{shotton-kinect} synthetise a training set of depth images which is then used to learn a keypoint regressor. An unfortunate byproduct of this training style however is the reliance on a depth sensor at test time. Another category of approaches are found in optical flow. Optical flow is an example of a dense point correspondence task: given two frames $I_{i}, I_{i+1}$ a model should predict a \emph{flow field} $f: \RR{H}{W} \mapsto \RR{H}{W}$. Of course, for this task the most important learning signal is obtained through observing object and camera \emph{motion}. For this reason, multiple models are trained using the FlyingChairs dataset: since natural scenes are complex to generate, models can just learn from flow directly. % Tidy up this argument

The alternative, used in this work, is to generate 2D silhouette images so that machine learning will predict joint heatmaps from silhouettes only.

A recent option for synthetic image generation is that of generative adversarial networks. 

\subsection{Handling silhouette ambiguity}

An unfortunate drawback of using silhouette images (for example, when compared to RGB or depth counterparts) is the missing interior contours leads to a source of reconstruction ambiguity. An example of this is shown in Figure 6 in which two distinct 3D reconstructions are shown to yield similar 2D silhouette reprojections. Clearly, a naive joint predictor which regreses a single set of 2D coordinates from an input silhouette would perform suboptimally, since the silhouette alone contains insufficient information to resolve such cases. For this reason, 
...
\section{Joint prediction using synthetic data}
...
\section{Optimal joint assignment (OJA)}
\label{sec:qp}
